...
Assuming that this base model and the model I constructed are the two best models we can make out of this dataset since the base model had the least AIC and the model I constructed had the most sense in regard to the problem we are dealing, I can conclude that there is no better model that could have achieved the 3 assumption checks while simultaneously following the narrative that we are aiming for. In addition, too much time will be spent if I were to try every combination of predictors and redo all the assumption tests.

One possible way to make this model a better model is to add more data to the data set. With more data, more can be accurately gauged by the model, leading to even more reliable predicted data.

With that being said, while it is important to constantly tweak a model in order to bring out the best possible outcome, it is vital to understand that no prediction can be 100% accurate and no model can ever be flawless. After all, life is as such and no amount of computation will be robust enough to predict a target completely accurately.
